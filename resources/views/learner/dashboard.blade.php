...
<div class="ea-welcome-card">
    ...
    <div class="ea-welcome-text">
        ...
        <div class="ea-welcome-meta">
            ...
            <span><i class="bi bi-mortarboard"></i> Classe : {{ $learner->schoolClass->name ?? '—' }}</span>
...
<div class="ea-stats-grid">
    ...
    <div class="ea-stat-card">
        ...
        <div><div class="ea-stat-value">{{ $announcements->count() }}</div><div class="ea-stat-label">Annonce(s) récente(s)</div></div>
    </div>
</div>
    ...
    <div class="ea-widget">
        ...
        <div class="ea-widget-body">
            ...
            <div class="ea-schedule-row">
                <div class="ea-schedule-day">{{ strtoupper(substr($creneau->day_of_week, 0, 3)) }}</div>
                <div class="ea-schedule-info"><strong>{{ substr($creneau->start_time,0,5) }} - {{ substr($creneau->end_time,0,5) }} · {{ $creneau->course->name ?? '' }}</strong><span class="ea-schedule-room">{{ $creneau->room }}</span></div>
            </div>
            @empty
    ...
    <div class="ea-widget">
        ...
        <div class="ea-widget-body">
            ...
            <div class="ea-resource-item">
                ...
                <div class="ea-resource-info"><strong>{{ $assignment->title }}</strong><span>{{ $assignment->course->name ?? '' }} · à rendre le {{ $assignment->due_date->format('d/m/Y') }}</span></div>
            </div>
            @empty
...
<div class="ea-widget">
    ...
    <div class="ea-widget-body">
        <div class="ea-actions-grid">
            ...
            <a href="{{ route('learner.documents.index') }}" class="ea-action-btn"><i class="bi bi-file-earmark-text"></i> Mes documents</a>
