...
@section('content')
<section class="subpage-hero" style="background-image:url('/images/actualites-hero.jpg'), linear-gradient(135deg, var(--royal-blue), var(--royal-blue-light));background-size:cover;background-position:center;position:relative;">
    ...
    <div class="container" style="position:relative;">
        ...
        <p>Restez informés des dernières actualités, événements et réalisations qui rythment la vie de la communauté Ambassadors.</p>
    </div>
</section>
        ...
        <div class="news-layout">
            ...
            <div>
                ...
                <div class="news-featured">
                    @if($featured->image)
                    <img src="{{ Storage::url($featured->image) }}" alt="{{ $featured->title }}" class="news-featured-img">
                    @else
                    ...
                    <div class="news-featured-body">
                        ...
                        <p style="color:var(--grey-mid);font-size:0.9rem;margin-bottom:12px;">{{ \Illuminate\Support\Str::limit($featured->content, 200) }}</p>
                        <a href="{{ route('actualites.show', $featured->id) }}" style="color:var(--royal-blue);font-weight:700;font-size:0.85rem;text-decoration:none;">Lire la suite <i class="bi bi-arrow-right"></i></a>
                    </div>
                </div>
                @endif

                <h3 style="font-family:var(--font-display);color:var(--royal-blue);margin-bottom:18px;">Dernières actualités</h3>
                ...
                <div class="news-grid">
                    ...
                    <div class="news-card">
                        ...
                        <div class="news-body">
                            ...
                            <div class="news-date"><i class="bi bi-calendar3"></i> {{ $item->published_at->format('d/m/Y') }}</div>
                        </div>
                    </div>
                    @endif
                    @endforeach
            ...
            <div>
                <div class="news-sidebar-widget">
                    ...
                    <div class="event-mini">
                        <div class="event-mini-date">
                            <span class="day">{{ $evenement->event_date->format('d') }}</span>
                            <span class="month">{{ $evenement->event_date->translatedFormat('M') }}</span>
                        </div>
                        <div class="event-mini-info">
                            <strong>{{ $evenement->icon }} {{ $evenement->title }}</strong>
                            <span>{{ $evenement->event_time }} @if($evenement->location) · {{ $evenement->location }} @endif</span>
                        </div>
                    </div>
                    @empty
                    <div style="padding:16px 20px;color:var(--grey-mid);font-size:0.82rem;">Aucun événement programmé.</div>
                    @endforelse
                </div>

                <div class="news-sidebar-widget">
                    <div class="mini-calendar">
                        <div class="mini-calendar-head">{{ \Illuminate\Support\Carbon::now()->translatedFormat('F Y') }}</div>
                ...
                <div class="news-share-cta">
                    ...
                    <a href="{{ route('contact') }}" class="btn btn-primary" style="width:100%;display:block;">Nous contacter <i class="bi bi-arrow-right"></i></a>
                </div>
            </div>
        </div>
    </div>
