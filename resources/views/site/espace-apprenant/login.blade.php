...
<body style="background:linear-gradient(135deg, var(--royal-blue), var(--royal-blue-light));min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;">
    ...
    <div class="admin-login-box" style="position:relative;">
        <a href="{{ route('home') }}" style="position:absolute;top:16px;left:16px;color:var(--grey-mid);font-size:0.8rem;text-decoration:none;"><i class="bi bi-arrow-left"></i> Retour au site</a>

        <div class="admin-login-logo"><i class="bi bi-mortarboard-fill" style="color:var(--gold);"></i></div>
        <h2 class="admin-login-title">Espace Apprenant</h2>
        <p class="admin-login-sub">Connectez-vous avec votre matricule</p>
        ...
        <form action="{{ route('espace-apprenant.login.post') }}" method="POST" style="margin-top:20px;">
            @csrf
            <div class="admin-form-group">
                <label class="admin-form-label">Matricule</label>
                <input type="text" name="matricule" class="admin-form-input" placeholder="AMB2024-0578" value="{{ old('matricule') }}" required autofocus>
            </div>
            <div class="admin-form-group">
