...
<div class="admin-topbar">
    <div>
        ...
        <p>{{ $facture->apprenant->first_name ?? '' }} {{ $facture->apprenant->last_name ?? '' }} — {{ $facture->libelle }}</p>
    </div>
</div>
...
<div class="admin-stats-grid">
    <div class="admin-stat-card">
        ...
        <div class="admin-stat-value">{{ number_format($facture->montant_total, 0, ',', ' ') }}</div>
    </div>
    <div class="admin-stat-card green">
        <div class="admin-stat-label">Payé</div>
        <div class="admin-stat-value">{{ number_format($facture->montant_paye, 0, ',', ' ') }}</div>
    </div>
    <div class="admin-stat-card red">
        <div class="admin-stat-label">Restant</div>
        <div class="admin-stat-value">{{ number_format($facture->montant_restant, 0, ',', ' ') }}</div>
    </div>
</div>
...
<div class="admin-panel" style="margin-bottom:24px;max-width:500px;">
    ...
    <div style="padding:20px;">
        <form action="{{ route('admin.factures.paiement', $facture->id) }}" method="POST">
            @csrf
            <div class="admin-form-group">
                <label class="admin-form-label">Montant (FCFA)</label>
                <input type="number" name="montant" class="admin-form-input" required>
            </div>
            <div class="admin-form-group">
                <label class="admin-form-label">Date du paiement</label>
                <input type="date" name="date_paiement" class="admin-form-input" value="{{ now()->format('Y-m-d') }}" required>
            </div>
            <div class="admin-form-group">
                <label class="admin-form-label">Mode de paiement</label>
                <select name="mode_paiement" class="admin-form-input">
            ...
            <div class="admin-form-group">
                ...
                <input type="text" name="reference_transaction" class="admin-form-input">
            </div>
